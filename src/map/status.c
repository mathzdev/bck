...
void initChangeTables(void)
{
	...
	add_sc(MH_SILENT_BREEZE		, SC_SILENCE	);
	...
	set_sc( SU_SCAROFTAROU		, SC_SCAROFTAROU	, SI_SCAROFTAROU	, SCB_NONE );
...
// ----- Power of Summoner Skills --------
	if((skill=pc_checkskill(sd,SU_POWEROFLIFE))>0) {
		status->flee += 20;
		status->hit += 20;
		status->cri += 20;
...
int status_change_start(struct block_list* src, struct block_list* bl,enum sc_type type,int rate,int val1,int val2,int val3,int val4,int tick,unsigned char flag) {
	...
	case SC_SCAROFTAROU:
		if(sc->opt1 || sc->data[SC_FREEZING] || sc->data[SC_BURNING])
			return 0;
	break;
	...
	case SC_WHITEIMPRISON:
		...
		status_change_end(bl, SC_FREEZE, INVALID_TIMER);
		status_change_end(bl, SC_STONE, INVALID_TIMER);
		status_change_end(bl, SC_SCAROFTAROU, INVALID_TIMER);
		break;
	case SC_FREEZING:
	case SC_SCAROFTAROU:
		status_change_end(bl, SC_BURNING, INVALID_TIMER);
	...
	switch(type) {
		...
		case SC_SCAROFTAROU: sc->opt1 = 0; break;
		// OPT2
		...
		case SC_WHITEIMPRISON:
			{
				...
				status_fix_damage(src,bl,400*sce->val1,clif_damage(bl,bl,gettick(),0,0,400*sce->val1,0,DMG_NORMAL,0),WL_WHITEIMPRISON);
			}
			break;
		case SC_WUGDASH:
			{
		...
		case SC_H_MINE:
			{
				...
				it.identify = 1;
				map_addflooritem(&it, it.amount, bl->m,bl->x, bl->y, caster->status.char_id, 0, 0, 0, 4);
			}
			break;
	}
...
#define sc_timer_next(t,f,i,d) \
	...
	case SC_SCAROFTAROU:
		if( --(sce->val4) >= 0 ) {
			struct block_list *src = map_id2bl(sce->val3);
			int damage = (10 * status_get_max_hp(bl)) / 100; // Deals 10% of HP
	...
	case SC_OVERHEAT:
		{
			...
			map_freeblock_lock();
			status_fix_damage(NULL,bl,damage,clif_damage(bl,bl,tick,0,0,damage,0,DMG_NORMAL,0),0);
			if( sc->data[type] ) {
				sc_timer_next(1000 + tick, status_change_timer, bl->id, data);
			}
			map_freeblock_unlock();
