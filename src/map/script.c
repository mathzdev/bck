...
BUILDIN_FUNC(callshop)
{
	...
	if( !nd || nd->bl.type != BL_NPC || (nd->subtype != NPCTYPE_SHOP && nd->subtype != NPCTYPE_CASHSHOP && nd->subtype != NPCTYPE_ITEMSHOP && nd->subtype != NPCTYPE_POINTSHOP && nd->subtype != NPCTYPE_SPSHOP) )
	{
...
BUILDIN_FUNC(waitingroom2bg)
{
	...
	struct chat_data *cd;
	const char *map_name, *ev = "", *dev = "";
	int x, y, mapindex = 0, bg_id, guild_index, n;
	unsigned char i;
