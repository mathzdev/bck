...
unsigned int SCDisabled[SC_MAX]; // [DanielArt]
	...
	if (type == STATUS_BONUS_FIX) {
		...
		if (bl->type == BL_PC) {
			...
			if ((sd->class_&MAPID_UPPERMASK) == MAPID_SUPER_NOVICE && sd->status.base_level >= 99)
				bonus += 3000; // Supernovice lvl99 hp bonus.
...
int status_change_start(struct block_list* src, struct block_list* bl,enum sc_type type,int rate,int val1,int val2,int val3,int val4,int tick,unsigned char flag) {
	...
	if(status->mode&MD_BOSS && !(flag&SCSTART_NOAVOID)) {
		if (type>=SC_COMMON_MIN && type <= SC_COMMON_MAX)
			return 0;
		switch (type) {
			case SC_ANKLE: // Ankle Snare don't work with MvP [DanielArt]
			case SC_BLESSING:
			case SC_DECREASEAGI:
			case SC_PROVOKE:
			case SC_COMA:
		...
		case SC_CLOSECONFINE:
			if (sce->val2 > 0) {
				...
				int range = 1
					...
					+ skill_get_range2(bl, TF_BACKSLIDING, 1, true); // Since most people use this to escape the hold....				map_foreachinarea(status_change_timer_sub,
...
#define sc_timer_next(t,f,i,d) \
	...
	case SC_STONE:
		...
		if(--(sce->val3) > 0) {
			sc_timer_next(100+tick,status_change_timer, bl->id, data );			return 0;
		}
		break;
